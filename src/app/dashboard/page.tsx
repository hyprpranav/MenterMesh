...
function DashboardContent() {
  ...
  useEffect(() => {
    async function loadAll() {
      ...
      try {
        ...
        setMyTeams(mTeams); setStaffUsers(staffList);
      } catch (err) {
  ...
  if (user.role === "master") {
    return <MasterDashboard user={user} students={students} staffUsers={staffUsers} teams={teams} events={events} requests={requests} announcements={announcements} loading={loadingData} />;
  }
  if (user.role === "staff") {
    return <StaffDashboard user={user} students={students} teams={teams} events={events} requests={requests} announcements={announcements} loading={loadingData} />;
  }
  return <StudentDashboard user={user} myTeams={myTeams} events={events} announcements={announcements} posts={posts} loading={loadingData} />;
}
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      {loading ? <LoadingState /> : (
        <>
          ...
          <StaffDashboard user={user} students={students} teams={teams} events={events} requests={requests} announcements={announcements} loading={false} />
        </>
      )}
    </div>
  );
